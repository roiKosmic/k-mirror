...
class HandRecognizer {
    ...
    public static void main(String[] args) {
       ...
       // cap.set(Videoio.CAP_PROP_FPS,5);
        ...
        boolean backgroundLearnt = false;
        while (cap.read(im)) {
        	//System.out.println(cap.get(Videoio.CAP_PROP_FPS));
              if (cascade != null ) {
            	 if(learnBg > 0 && !backgroundLearnt) {
              		bg.apply(im, fg);
              		learnBg--;
              		...
              		if(frameSinceBgLearnt > Integer.parseInt(Config.getConfig("BACKGROUND_UPDATE_FACTOR"))*Integer.parseInt(Config.getConfig("BACKGROUND_LEARN"))  && nothingDetected && backgroundLearnt){
              			learnBg = Integer.parseInt(Config.getConfig("BACKGROUND_LEARN"));
              			System.out.println("Background refresh request");
              			bg.apply(im, fg,1);
              			backgroundLearnt = false;
              			frameSinceBgLearnt=0;
              			
              		}else{
              			bg.apply(im, fg,0);
                  		if(learnBg==0){
                  			backgroundLearnt = true;
                  			//System.out.println("bglearnt");
                  			frameSinceBgLearnt++;
                  		}
              		}
